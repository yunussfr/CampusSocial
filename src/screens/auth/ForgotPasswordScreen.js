        ...
        <View style={styles.card}>
          <View style={styles.illustration}>
            <View style={styles.envelope}>
              <MdiIcon path={mdiLockReset} size={58} color={COLORS.primary} />
            </View>
          </View>
          ...
          {sent ? (
            <Text style={styles.successText}>Sıfırlama e-postası gönderildi.</Text>
          ) : null}
          {error ? <Text style={styles.errorText}>{error}</Text> : null}

          <PrimaryButton disabled={submitting} onPress={handleSubmit}>
            {submitting ? <ActivityIndicator color="#FFFFFF" /> : 'Gönder'}
          </PrimaryButton>
        </View>

        <Pressable onPress={() => navigation.navigate('Login')} style={styles.bottomLink}>
          <MdiIcon path={mdiArrowLeft} size={28} color={COLORS.primary} />
